...
def _testForAppleProjectileCollision(projectile, apples):
    for apple in apples[:]:
...
def _moveProjectile(direction, projectile):
    dx = direction.getX()
    dy = direction.getY()
...
def update(projectiles, projectileDirections, apples):
    ...
    for i in range(len(projectiles)):
        _moveProjectile(projectileDirections[i], projectiles[i])
        _testForAppleProjectileCollision(projectiles[i], apples)
